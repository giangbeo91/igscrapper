...
namespace IgScrapper;

use IgScrapper\Exception\InstagramAuthException;
use IgScrapper\Exception\InstagramException;
use IgScrapper\Exception\InstagramNotFoundException;
use IgScrapper\Model\Account;
use IgScrapper\Model\Comment;
use IgScrapper\Model\Like;
use IgScrapper\Model\Location;
use IgScrapper\Model\Media;
use IgScrapper\Model\Story;
use IgScrapper\Model\Tag;
use IgScrapper\Model\UserStories;
...
class Instagram
{
    ...
    public function getMediaCommentsByCode($code, $count = 10, $maxId = null)
    {
        ...
        while ($hasPrevious && $index < $count) {
            ...
            static::parseCookies($response->headers);
            $jsonResponse = $this->decodeRawBodyToJson($response->raw_body);
    ...
    private function parseCookies($headers)
    {
        ...
        $rawCookies = isset($headers['Set-Cookie']) ? $headers['Set-Cookie'] : [];
